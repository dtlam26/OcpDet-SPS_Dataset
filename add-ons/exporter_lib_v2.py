...
class DetectionInferenceModule(tf.Module):
  ...
  def _run_inference_on_images(self, images, true_shapes, **kwargs):
    ...
    """
    if 'skip_keys' in kwargs:
        skip_keys = kwargs['skip_keys']
        kwargs.pop('skip_keys',None)
    else:
        skip_keys = []
    if 'skip_postprocess' in kwargs:
        skip_postprocess = kwargs['skip_postprocess']
        kwargs.pop('skip_postprocess',None)
    else:
    ...
    for key, val in detections.items():
      if key in skip_keys:
          continue
      print(key)
      if key != 'active_anchors':
          full_detections[key] = tf.cast(val, tf.float32)
      else:
          for anchor in detections[key]:
              if type(anchor)==list:
                  full_detections[f'num_anchors'] = tf.constant(anchor)
              else:
                  full_detections[f'{key}_{i}'] = tf.cast(anchor, tf.float32)
                  i = i + 1  
    if i>0:
      try:
          full_detections.pop(f'active_anchors_{i-1}')
      except:
          pass
...
class DetectionFromFixImageModule(DetectionInferenceModule):
  """Detection Inference Module for fix shape image inputs."""
  def __init__(self, detection_model,
               use_side_inputs=False,
               zipped_side_inputs=None):
    global IMAGE_SIZE
    global TENSOR_MODE
    print(f"Export on Fixed Shape: {IMAGE_SIZE[0], IMAGE_SIZE[1]}")
    input_tensor = tf.random.uniform(shape=[1, IMAGE_SIZE[0], IMAGE_SIZE[1], 3],
                             dtype=tf.float32,
                             name='input_tensor')

    true_shapes = tf.constant([[IMAGE_SIZE[0], IMAGE_SIZE[1], 3]])

    def call_func(input_tensor):
      if TENSOR_MODE=='jetson_tensor':
          return self._run_inference_on_images(input_tensor, true_shapes,
                        skip_keys=['preprocessed_inputs','feature_maps','final_anchors','anchors'],
                        skip_postprocess=True)
      elif TENSOR_MODE=='fixed_tensor':
          return self._run_inference_on_images(input_tensor, true_shapes,
                        skip_keys=['num_detections','raw_detection_boxes', 'raw_detection_scores',
                                   'detection_keypoints','detection_multiclass_scores'])

    self.__call__ = tf.function(call_func, input_signature=[
              tf.TensorSpec(shape=[1, IMAGE_SIZE[0], IMAGE_SIZE[1], 3], dtype=tf.float32)])
    super(DetectionFromFixImageModule, self).__init__(detection_model,
                                                   use_side_inputs,
                                                   zipped_side_inputs)
...
def export_inference_graph(input_type,
  ...
  if input_type in ['fixed_tensors','jetson_tensor']:
    global TENSOR_MODE
    TENSOR_MODE = input_type
    
    global IMAGE_SIZE
    try:
        IMAGE_SIZE = (
              int(pipeline_config.model.ssd.image_resizer.fixed_shape_resizer.height),
              int(pipeline_config.model.ssd.image_resizer.fixed_shape_resizer.width)
          )
    except:
        print(pipeline_config.model.__dict__)
...
DETECTION_MODULE_MAP = {
    ...
    'jetson_tensor': DetectionFromFixImageModule,
    'fixed_tensor': DetectionFromFixImageModule
}
