...
class Learning_Loss(Loss):
  """Self implement learning loss"""
  def __init__(self,margin=1.0):
    self.margin = margin
    
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    batch_size = len(prediction_tensor)
    
    prediction_tensor = (prediction_tensor - tf.reverse(prediction_tensor,[0]))[:batch_size//2] 
    target_tensor = (target_tensor - tf.reverse(target_tensor,[0]))[:batch_size//2]
    one = 2 * tf.math.sign(tf.clip_by_value(target_tensor, 0,9999)) - 1
    return tf.identity(tf.reduce_mean(tf.clip_by_value(self.margin - one * prediction_tensor, 0, 9999))*weights,name="learning_loss")

class Active_Anchors_Loss(Loss):
  def __init__(self,margin=0.1):
    self.margin = margin
    
  def _compute_loss(self, prediction_tensors, target_tensor,weights=1e4):
    start_slice = 0
    end_slice = 0
    # candidate = tf.expand_dims(
    #         tf.clip_by_value(tf.math.reduce_max(target_tensor[:,:,1:],-1),0,0.8),-1
    # )
    num_classes = list(target_tensor.get_shape())[-1]-1
    partial_loss = tf.zeros([])
    for i in range(len(prediction_tensors[:-1])):
        prediction_tensor = prediction_tensors[i]
        shape = list(prediction_tensor.get_shape())
        current_num_anchors = shape[1]*shape[2]*prediction_tensors[-1][i]
        end_slice += current_num_anchors
        slice_target_tensor = target_tensor[:,start_slice:end_slice,1:]
        #Tensor (BxNxNx1)
        new_target = tf.math.reduce_max(
            tf.reshape(slice_target_tensor,(-1,shape[1],shape[2],num_classes*prediction_tensors[-1][i])),-1,keepdims=True
        )
        partial_loss += tf.reduce_sum(prediction_tensor - new_target - self.margin)**2
        start_slice = end_slice
    
    return tf.identity((tf.clip_by_value(partial_loss,0,1e9)/end_slice)*weights,name="active_anchors_loss")
...
class WeightedSmoothL1LocalizationLoss_Diamond(Loss):
  """Custom Loss for Parking ..
  ...
  def __init__(self, delta=1.0):
    ...
    Args:
      delta: delta for smooth L1 loss.
    """
    super(WeightedSmoothL1LocalizationLoss_Diamond, self).__init__()
    self._delta = delta
  
  def _diamond_distance(self,center_point, diamond_points, target_diamond_points, batch_size):
    y = diamond_points[:,:,::2]
    x = diamond_points[:,:,1::2]
    target_y = target_diamond_points[:,:,::2]
    target_x = target_diamond_points[:,:,1::2]
    
    center_y = tf.reshape(center_point[:,:,0],(batch_size,-1,1))
    center_x = tf.reshape(center_point[:,:,1],(batch_size,-1,1))
    center_distance = -tf.reduce_mean((x- center_x+1e-6)**2 + (y-center_y+1e-6)**2,-1)
    corner_distance = tf.reduce_mean((x- target_x)**2 + (y-target_y)**2,-1)
    return (corner_distance/center_distance)*0.000005
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    batch_size = len(prediction_tensor)
    _regulizer = self._diamond_distance(prediction_tensor[:,:,:2],prediction_tensor,
                                   target_tensor,batch_size)
    return tf.reduce_sum(tf.losses.huber_loss(
        target_tensor,
        prediction_tensor,
        delta=self._delta,
