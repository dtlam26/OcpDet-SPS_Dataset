...
class LearningLoss_module(tf.keras.Model):
  # This is the default config for mobilenet
  def __init__(self,conv_hyperparams=None,layer_kernels=[19,10,5,3,2,1],depth=[576,1280,512,256,256,128]):
    super().__init__()
    def GAP(x,i):
        if type(x) == int:
            assert depth[i]>0, "Not allow for naive depth like -1"
            kernel = x
            f_shape = [None,kernel,kernel,depth[i]]
        else:
            f_shape = list(x.get_shape())
            kernel = int(f_shape[1])
        input_x = tf.keras.layers.Input(shape=f_shape[1:])
        x = tf.keras.layers.AveragePooling2D((kernel,kernel),name=f"GAP_avg_{i}")(input_x)
        x = tf.keras.layers.Flatten(name=f"GAP_flat_{i}")(x)
        x = tf.keras.layers.Dense(128,name=f"GAP_dense_{i}",**conv_hyperparams.params())(x)
        x = conv_hyperparams.build_activation_layer(name=f"GAP_dense_{i}_activation")(x)
        return tf.keras.Model(input_x, x)
    
    self.GAP_components = []
    for i,layer_kernel in enumerate(layer_kernels):
        self.GAP_components.append(GAP(layer_kernel,i))
    self.last_index = len(layer_kernels)
    self.GAP_concat = tf.keras.layers.Concatenate(axis=1,name="GAP_concat")
    if conv_hyperparams is not None:
        self.ll = tf.keras.layers.Dense(1,name="ll_pred",**conv_hyperparams.params())
    else:
        self.ll = tf.keras.layers.Dense(1,name="ll_pred")
        
  def call(self, feature_maps):
    tempt = []
    for i in range(len(feature_maps[:self.last_index])):
        tempt.append(self.GAP_components[i](feature_maps[i]))
    return self.ll(self.GAP_concat(tempt))


class ActiveAnchors_module(tf.keras.Model):
  def __init__(self,active_anchors,image_size,layer_kernels,depth,conv_hyperparams=None,
               is_training=False,freeze_batchnorm=False):
    super().__init__()
    
    def active_anchor_head(head,i):
        num_anchors = int(((image_size / head['info'][-1][0])**2)*len(head['info'][-2])*len(head['info'][-3]))
        # print(head['info'])
        assert depth[i]>0, "Not allow for naive depth like -1"
        f_shape = [None,layer_kernels[i],layer_kernels[i],depth[i]]
            
        input_x = tf.keras.layers.Input(shape=f_shape[1:])
        
        addition_x = tf.keras.layers.Conv2D(
              64,
              [1, 1],
              padding='SAME',
              name=f"AA_residual_{i}",
              **conv_hyperparams.params())(input_x)
        
        addition_x = conv_hyperparams.build_batch_norm(
                  training=(is_training and not freeze_batchnorm),
                  name=f"AA_residual_bn_{i}")(addition_x)
        addition_x = conv_hyperparams.build_activation_layer(
                  name=f"AA_residual_act_{i}")(addition_x)
        ...
        x = tf.keras.layers.Conv2D(
              ...
              padding='SAME',
              strides=2,
              name=f"AA_downsample_{i}",
              **conv_hyperparams.params())(input_x)
        
        x = conv_hyperparams.build_batch_norm(
                  training=(is_training and not freeze_batchnorm),
                  name=f"AA_downsample_bn_{i}")(x)
        x = conv_hyperparams.build_activation_layer(
                  name=f"AA_downsample_act_{i}")(x)
        x = tf.keras.layers.Conv2D(
              64,
              [1, 1],
              padding='SAME',
              name=f"AA_down_channel_{i}",
              **conv_hyperparams.params())(x)
        x = conv_hyperparams.build_batch_norm(
                  training=(is_training and not freeze_batchnorm),
                  name=f"AA_down_channel_bn_{i}")(x)
        x = conv_hyperparams.build_activation_layer(
                  name=f"AA_down_channel_act_{i}")(x)
        x = tf.keras.layers.UpSampling2D(
              [2, 2],
              name=f"AA_upsample_{i}")(x)
        
        if layer_kernels[i]%2==1:
            padding='VALID'
        else:
            padding='SAME'
        ...
        x = conv_hyperparams.build_batch_norm(
                  training=(is_training and not freeze_batchnorm),
                  name=f"A_conv_bn_{i}")(x)
        x = tf.keras.layers.Add()([x, addition_x])
        x = tf.reduce_mean(x,-1,keepdims=True)
        x = tf.keras.activations.sigmoid(x)
        
        # x = conv_hyperparams.build_activation_layer(name=f"AA_dense_act_{i}")(x)
        # x = tf.clip_by_value(
        #     x, 0, 1, name=f"AA_mask_{i}"
        # )
        # flatten = tf.keras.layers.Flatten(name=f"AA_dense_{i}")(x)
        
        
        return tf.keras.Model(input_x, x),len(head['info'][-2])*len(head['info'][-3])
        
    
    self.last_index = len(active_anchors)
    
    self.anchor_heads = []
    self.num_anchors = []
    for i,head in enumerate(active_anchors):
        anchor_head, num_anchors = active_anchor_head(head,i)
        self.anchor_heads.append(anchor_head)
        self.num_anchors.append(num_anchors)
    
    # self.aa = tf.keras.layers.Concatenate(axis=1,name="AA_pred")
    # self.expand = tf.expand_dims
        
  def call(self, feature_maps):
    tempt = []
    # merge = []
    for i in range(len(feature_maps[:self.last_index])):
        head = self.anchor_heads[i](feature_maps[i])
        tempt.append(head)
        # merge.append(flatten)
    # tempt.append(self.expand(self.aa(merge),-1))
    tempt.append(self.num_anchors)
    return tempt
...
class KerasMultiResolutionFeatureMaps(tf.keras.Model):
  ...
  def __init__(self,
    ...
    self.conv_hyperparams = conv_hyperparams
    ...
    for index, from_layer in enumerate(feature_map_layout['from_layer']):
      ...
      if from_layer:
        ...
        if insert_1x1_conv:
          if 'corresponding_base' in feature_map_layout:
            if feature_map_layout['corresponding_base'][index]:
                base_from_layer = feature_map_layout['corresponding_base'][index]
      ...
      print("Additional Layers: ", [n.name for n in net])
      
      self.convolutions.append(net)
      if 'layer_kernel' in feature_map_layout:
          self.learning_loss = LearningLoss_module(conv_hyperparams,feature_map_layout['layer_kernel'],
                                        feature_map_layout['layer_depth'])
      if 'active_anchors' in feature_map_layout:
          self.active_anchors = ActiveAnchors_module(feature_map_layout['active_anchors'],
                                        feature_map_layout['image_size'],feature_map_layout['layer_kernel'],
                                        feature_map_layout['layer_depth'],conv_hyperparams,
                                        is_training,freeze_batchnorm)
            
            
  def call(self, image_features, learning_loss=False):
    ...
    for index, from_layer in enumerate(self.feature_map_layout['from_layer']):
      if from_layer:
        ...
        if index < len(image_features):
            feature_map = list(image_features.values())[index]
        else:
...
#     if learning_loss:
#       if self.learning_loss is None:
        #avoid reinitialize when inference
#         self.learning_loss = LL_module(self.conv_hyperparams,feature_maps)
    if self.learning_loss is not None:
        feature_collection['learning_loss'] = self.learning_loss(feature_maps)
...
class KerasFpnTopDownFeatureMaps(tf.keras.Model):
  ...
  def __init__(self,
    ...
    if features_attr:
        self.learning_loss = LearningLoss_module(conv_hyperparams,features_attr['layer_kernel'],
                                        features_attr['layer_depth'])
        if 'active_anchors' in features_attr:
          self.active_anchors = ActiveAnchors_module(features_attr['active_anchors'],
                                        features_attr['image_size'],features_attr['layer_kernel'],
                                        features_attr['layer_depth'],conv_hyperparams,
                                        is_training,freeze_batchnorm)
  ...
  def extract_learningloss(self,feature_maps_list):
    assert self.learning_loss, 'Please describe the feature kernel and depth in order at features_attr'
    return self.learning_loss(feature_maps_list)
    
  def extract_anchor_interest(self,feature_maps_list):
    assert self.active_anchors, 'Please describe the feature kernel and depth in order at features_attr'
    return self.active_anchors(feature_maps_list)
  ...
  def call(self, image_features,learning_loss=False):
    ...
    feature_collection = collections.OrderedDict(reversed(
        list(zip(output_feature_map_keys, output_feature_maps_list))))
    
    # if self.learning_loss is not None:
    #     feature_collection['learning_loss'] = self.extract_learningloss(output_feature_maps_list)
    return feature_collection
