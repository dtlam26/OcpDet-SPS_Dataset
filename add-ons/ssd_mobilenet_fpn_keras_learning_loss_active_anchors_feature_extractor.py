...
class SSDMobileNetV2FpnLearningLossKerasFeatureExtractor(
    ssd_meta_arch.SSDKerasFeatureExtractor):
  """SSD Feature Extractor using Keras-based MobilenetV2 FPN features with learning loss."""
  ...
  def __init__(self,
    ...
    """
    super(SSDMobileNetV2FpnLearningLossKerasFeatureExtractor, self).__init__(
        ...
        name=name)
    if active_anchors:
        self.anchor_info = active_anchors._anchor_grid_info
    else:
        self.anchor_info = []
    ...
    self.features_attr = {}
    # self.features_attr = {
    #         'layer_depth': [self._additional_layer_depth]*5
    # }
  ...
  def build(self, input_shape):
    expected_num_levels = self._fpn_max_level + 1 - self._fpn_min_level
    ...
    if input_shape[1] == input_shape[2]:
        if self.anchor_info:
            kernel = input_shape[1]/self.anchor_info[0]['info'][-1][0]
        else:
            kernel = input_shape[1]/8
            
        assert kernel == int(kernel), "Please choose another input size!"
        
        layer_kernel = [int(kernel)]
        for _ in range(expected_num_levels-1):
            layer_kernel.append(int(layer_kernel[-1]/2))
            
        self.features_attr['layer_kernel'] = layer_kernel
        self.features_attr['layer_depth'] = [self._additional_layer_depth]*expected_num_levels
        if self.anchor_info:
            self.features_attr['active_anchors'] = self.anchor_info
            self.features_attr['image_size'] = input_shape[1]
    else:
    ...
    self._fpn_features_generator = (
        feature_map_generators.KerasFpnTopDownFeatureMaps(
            ...
            features_attr=self.features_attr,
            conv_hyperparams=self._conv_hyperparams,
  ...
  def _extract_features(self, preprocessed_inputs):
    ...
    # feature_maps.append(fpn_features['learning_loss'])
    
    ll = self._fpn_features_generator.extract_learningloss(feature_maps)
    
    if self.anchor_info:
        aa = self._fpn_features_generator.extract_anchor_interest(feature_maps)
    feature_maps.append(ll)
    
    if self.anchor_info:
        feature_maps.append(aa)
    return feature_maps
