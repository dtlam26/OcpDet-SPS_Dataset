...
class SSDMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    self._learning_loss = losses.Learning_Loss()
    self.learning_loss = False
    
    self._active_anchors_loss = losses.Active_Anchors_Loss()
    self.active_anchors = False
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):
    ...
    if self._feature_extractor.is_keras_model:
      ...
      if list(feature_maps[-2].get_shape())[-1] == 1:
        ll = feature_maps[-2]
        aa = feature_maps[-1]
        feature_maps = feature_maps[:-2]
        self.learning_loss = True
        self.active_anchors = True
        
      elif list(feature_maps[-1].get_shape())[-1] == 1:
        ll = feature_maps[-1]
        aa = None
    ...
    if aa is not None:
      predictions_dict.update({'active_anchors': aa})
      print("Keras_mode:",self._box_predictor.is_keras_model)
      if self._box_predictor.is_keras_model:
        predictor_results_dict = self._box_predictor(feature_maps,active_anchors=aa)
        # predictor_results_dict = self._box_predictor(feature_maps)
    #Predice and return flat box prediction
    else:
      if self._box_predictor.is_keras_model:
        predictor_results_dict = self._box_predictor(feature_maps)
      else:
    ...
    self._batched_prediction_tensor_names = [x for x in predictions_dict
                                             if x != 'anchors']
    if ll is not None:
        predictions_dict.update({'learning_loss': ll})
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    with tf.name_scope('Postprocessor'):
      ...
      if self.learning_loss:
        detection_dict.update({
            'learning_loss': prediction_dict['learning_loss'],
        })
      if self.active_anchors:
        detection_dict.update({
            'active_anchors': prediction_dict['active_anchors'],
        }) 
  ...
  def loss(self, prediction_dict, true_image_shapes, scope=None):
    ...
    Returns:
      a dictionary mapping loss keys (`localization_loss` and
        ...
        values.
    """
    print("Optimize those predictions")
    [print(k,v) for k,v in prediction_dict.items()]
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      if self.active_anchors:
          active_anchors_loss = self._active_anchors_loss(prediction_dict['active_anchors'],batch_cls_targets,weights=1e-8)
      ...
      if self.learning_loss:
        ...
        loss_dict.update({'Loss/learning_loss':
          self._learning_loss(prediction_dict['learning_loss'],
                              tf.reshape(tf.reduce_sum(location_losses,axis=1),(-1,1))+tf.reshape(tf.reduce_sum(cls_losses,axis=1),(-1,1)),
                              losses_mask=losses_mask,weights=.1)})
      if self.active_anchors:
        loss_dict.update({'Loss/active_anchors_loss': active_anchors_loss})
